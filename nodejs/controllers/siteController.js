...
const getTemplate = async (req, res, next) => {
  try {
    ...
    if(template_req.data.type != "blog" && template_req.data.type != "contact") {
          allFAQ = await getFAQ(template_req);
          howToSeo = await getHowTO(template_req);
          videoSeo = await getVideos(template_req);
          serviceSeo = await getService(template_req);
          linkJuice = await getLinkJuice(template_req);
          website = await getWebSite(template_req);
          webpage = await getWebPage(template_req);
    }
    if(template_req.data.type != "home") {
        breadcrumbs = await getBreadcrumbList(template_req);
    }
    if(template_req.data.type != "contact") {
       logo = getLogo(template_req);
       localBusinessSeo = await getLocalBusiness(template_req);
    }
    ...
    if(template_req.data.type == 'blog') {
      const bolgList = await getBlog();
      apiResponse["[blog_list]"] = bolgList;
    }
...
const genResJson = async (apiResponse, template_req) => {
  try {
    ...
    let tableName = "template_tag";
    let jsonResponse;    
    let sql = `SELECT * FROM ${tableName} WHERE type = ? OR type = ?`;
    let params = ["common", type];
    const template = await selectAll(sql,params);
    let jsonTemplateObject = {};
    if (template) {
      template.forEach((templateKeyValue) => {
        jsonTemplateObject[templateKeyValue.tag_name] = templateKeyValue.text;
      });
      
      jsonResponse =JSON.stringify(jsonTemplateObject);
...
const getMetaTag = async (template_req) => {
  let tableName = "meta_information";
  const { type } = template_req.data;
...
const getBlog = async () => {
  try {
    let tableName = "blog";
    let sql = `SELECT * FROM ${tableName}`;
    const blog =  await selectAll(sql);
    let blogArray = [];
